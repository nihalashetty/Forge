...
**Status (2026-06-19) — inline component ordering (components render in their natural place).** Fixed the reported bug where a rendered component always jumped to the TOP of the reply (never middle/end) regardless of how the question was framed. Root cause: a component renders as a side-effect of a tool call, which the ReAct agent runs BEFORE writing its prose (the prose is one final turn after every tool returns), and the client placed components in frame-arrival order — so the widget's frame always preceded all text. **Fix (the ChatGPT/Claude approach): decouple rendering from placement via an inline marker.** The component tool now returns a placeholder `[[forge:component:<instance_id>]]` in its ack ([tools/components.py](apps/api/forge/tools/components.py)) and `COMPONENT_STYLE` instructs the agent to copy that marker into its reply where the widget belongs ([agent_node.py](apps/api/forge/nodes/agent_node.py)) — so the agent controls order (mid-answer, after a heading, at the end), writing text before and after it. Props/markup still travel out-of-band on the custom stream (only the ~10-token marker is in the token stream). A shared parser ([apps/web/lib/chat-parts.ts](apps/web/lib/chat-parts.ts) — `ReplyAccumulator` + `assembleParts`) splits the assistant text on markers and splices each already-received component instance into place; it hides a half-streamed marker so it never flashes, withholds a component until its marker arrives, drops unknown/hallucinated markers, and on finalize appends any component the model rendered-but-never-marked at the END (never the top) so a widget is never lost. Wired into all three chat surfaces — [playground.tsx](apps/web/components/screens/playground.tsx), the embed widget [embed/page.tsx](apps/web/app/embed/page.tsx), and the canvas [WorkflowTestPanel.tsx](apps/web/components/canvas/WorkflowTestPanel.tsx). Text-only channels (email/Teams/webhook/schedule/evals via `run_to_completion`) strip the markers ([runs.py](apps/api/forge/services/runs.py) + `strip_component_markers`). Verified: backend imports clean + marker helpers correct; web builds + type-checks clean (Node 22); parser unit-traced over 8 edge cases (mid/end placement, partial-marker hiding, hallucinated id, dedup, interleaving, wiki-link false-positive). Still needs the running stack + an LLM to E2E the agent actually emitting markers.
