            ...
            <tr key={i}>
              <td>
                <div className="row gap2">
                  <Tile icon={r.kind === "assistant" ? "sparkles" : "workflows"} color={r.kind === "assistant" ? "var(--signal)" : "var(--accent)"} size={24} />
                  <div>
              ...
              {kpis.map((k, i) => (
                <div key={i} className="card" style={{ padding: 16 }}>
                  <div className="row spread" style={{ marginBottom: 8 }}>
                    ...
                    <span style={{ width: 7, height: 7, borderRadius: 2, background: k.color }} />
                  </div>
      ...
      <div className="fade-up" style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 18 }}>
          {health.map((h, i) => (
            <button key={i} className="card card-hover" style={{ padding: 16, textAlign: "left", background: "var(--bg-1)" }} onClick={() => onNav(h.screen)}>
              <div className="row spread"><Tile icon={h.icon} color={h.color} size={34} /><Icon name="chevright" size={16} style={{ color: "var(--fg-2)" }} /></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20 }}>
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            <div className="col gap3">
              {[["msg", "Channels", "Email / Microsoft Teams", "var(--accent)", "channels"], ["connect", "MCP Server", "Not exposed", "var(--signal)", "connect"], ["playground", "Playground", "Test your workflow", "var(--io-json)", "playground"]].map((d, i) => (
