...
export function ToolsScreen({ project, onOpen, focusSetId }: { project: any; onOpen: (t: Tool) => void; focusSetId?: string | null }) {
  ...
  async function del(e: React.MouseEvent, t: Tool) {
    e.stopPropagation();
    if (t.kind === "builtin") return; // built-ins are platform capabilities — not deletable
  ...
  const shown = useMemo(() => {
    ...
    // Built-ins (platform capabilities) always float to the top; stable within each group.
    return [...list].sort((a, b) => (a.kind === "builtin" ? 0 : 1) - (b.kind === "builtin" ? 0 : 1));
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div className="row gap2 wrap" style={{ marginTop: 16, alignItems: "center" }}>
                ...
                <ImportExport project={project} type="tool" typeLabel="tool" size="md" onImported={reload}
                  items={tools.filter((t) => t.kind !== "builtin").map((t) => ({ id: t.id, name: t.name, sub: KIND_LABEL[t.kind] || t.kind }))} />
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {shown.map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="row" style={{ cursor: "pointer", boxShadow: isBuiltin ? "inset 3px 0 0 var(--signal)" : undefined }} onClick={() => onOpen(t)}>
                          {showCheckbox && <td onClick={(e) => { e.stopPropagation(); toggleSelect(t.id); }}><Checkbox checked={selected.has(t.id)} /></td>}
                          <td><div className="row gap2"><Icon name={isBuiltin ? "lock" : (KIND_ICON[t.kind] || "k_rest")} size={15} style={{ color: isBuiltin ? "var(--signal)" : "var(--accent)" }} /><span className="mono-sm" style={{ fontWeight: 600 }}>{t.name}</span></div></td>
                          <td>{isBuiltin ? <span className="chip chip-mono" style={{ color: "var(--signal)", borderColor: "var(--signal)" }}>Built-in</span> : <span className="chip chip-mono">{KIND_LABEL[t.kind] || t.kind}</span>}</td>
                          ...
                          <td style={{ textAlign: "right" }}>
                            <div className="row gap1" style={{ justifyContent: "flex-end" }}>
                              ...
                              {isBuiltin
                                ? <button className="iconbtn" title="Built-in tool — protected" disabled onClick={(e) => e.stopPropagation()} style={{ opacity: 0.4, cursor: "default" }}><Icon name="lock" size={14} /></button>
      ...
      <div className="row" style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: 640, maxWidth: "96vw", background: "var(--bg-1)", borderLeft: "1px solid var(--line)", boxShadow: "var(--sh-pop)", transform: open ? "none" : "translateX(100%)", transition: "transform var(--dur-slow) var(--ease)", alignItems: "stretch" }}>
        ...
        <div className="col grow" style={{ minWidth: 0, minHeight: 0 }}>
          <div style={{ position: "relative", zIndex: 3, padding: "22px 24px 0" }}>
            ...
            <Field label="Name"><input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="crm_tools" /></Field>
...
function ToolCard({ t, sets, selectable, selected, onToggleSelect, memberSetIds, onToggleSet, onOpen, onDelete, onDuplicate, onToggle }: { t: Tool; sets: ToolSet[]; selectable: boolean; selected: boolean; onToggleSelect: () => void; memberSetIds: Set<string>; onToggleSet: (setId: string, isMember: boolean) => void; onOpen: () => void; onDelete: (e: React.MouseEvent) => void; onDuplicate: (e: React.MouseEvent) => void; onToggle: () => void }) {
  ...
  return (
    ...
    <div className="card card-hover" style={{ padding: 16, position: "relative", zIndex: menuOpen ? 30 : undefined, opacity: t.enabled ? 1 : 0.6, borderColor: selected ? "var(--accent)" : isBuiltin ? "var(--signal)" : undefined, boxShadow: isBuiltin ? "inset 3px 0 0 var(--signal)" : undefined }} onClick={onOpen}>
      <div className="row spread" style={{ marginBottom: 12 }}>
        <div className="row gap2">
          {selectable && <div onClick={(e) => { e.stopPropagation(); onToggleSelect(); }}><Checkbox checked={selected} /></div>}
          {isBuiltin
            ? <span className="chip chip-mono row gap1" style={{ color: "var(--signal)", borderColor: "var(--signal)" }}><Icon name="lock" size={11} style={{ color: "var(--signal)" }} />Built-in</span>
            : <span className="chip chip-mono">{KIND_LABEL[t.kind] || t.kind}</span>}
        </div>
        <div className="row gap2" style={{ alignItems: "center" }}>
          ...
          <ToolCardMenu enabled={t.enabled} canDelete={!isBuiltin} sets={sets} memberSetIds={memberSetIds} onToggleSet={onToggleSet} onToggle={onToggle} onDuplicate={onDuplicate} onDelete={onDelete} onOpenChange={setMenuOpen} />
        </div>
      </div>
...
function ToolCardMenu({ enabled, canDelete = true, sets, memberSetIds, onToggleSet, onToggle, onDuplicate, onDelete, onOpenChange }: { enabled: boolean; canDelete?: boolean; sets: ToolSet[]; memberSetIds: Set<string>; onToggleSet: (setId: string, isMember: boolean) => void; onToggle: () => void; onDuplicate: (e: React.MouseEvent) => void; onDelete: (e: React.MouseEvent) => void; onOpenChange?: (open: boolean) => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
    ...
    <div ref={ref} style={{ position: "relative" }} onClick={(e) => e.stopPropagation()}>
      ...
      {open && (
        <div className="card fade-in" role="menu" style={{ position: "absolute", top: "100%", right: 0, marginTop: 6, zIndex: 6000, minWidth: 190, padding: 4, boxShadow: "var(--sh-pop)" }}>
          ...
          {/* Show only the sets this tool actually belongs to (click a row to remove it). The full
              catalogue would be noise on a card; add-to-set lives in the Manage toolsets drawer. */}
          {(() => {
            const memberSets = sets.filter((s) => memberSetIds.has(s.id));
            return memberSets.length > 0 ? memberSets.map((s) => (
              <button key={s.id} role="menuitemcheckbox" aria-checked title="Remove from this tool set" style={{ ...item, color: "var(--fg-1)", justifyContent: "space-between" }} onClick={() => onToggleSet(s.id, true)}>
          ...
          {canDelete
            ...
            : <div style={{ ...item, color: "var(--fg-2)", cursor: "default" }}><Icon name="lock" size={15} style={{ color: "var(--signal)" }} />Built-in · protected</div>}
        </div>
      )}
    </div>
  );
...
// Arg schemas for builtin tools (their real schema lives in the Python pydantic models). Surfaced
// here so the Test panel renders typed inputs instead of a raw-JSON box, like REST/code/SQL tools.
const BUILTIN_ARG_FIELDS: Record<string, { path: string; type: string; required?: boolean; description?: string }[]> = {
  current_time: [{ path: "tz", type: "string", description: "Timezone (only UTC offline)" }],
  calculator: [{ path: "expression", type: "string", required: true, description: "Arithmetic expression" }],
  web_fetch: [{ path: "url", type: "string", required: true, description: "URL to fetch" }],
  web_search: [{ path: "query", type: "string", required: true, description: "Search query" }],
  knowledge_search: [
    { path: "query", type: "string", required: true, description: "What to look up in the project KB" },
    { path: "folder", type: "string", description: "Optional folder to search within (empty = all)" },
    { path: "top_k", type: "integer", description: "How many document chunks to return" },
  ],
  remember: [{ path: "text", type: "string", required: true, description: "A fact to remember" }],
  recall: [{ path: "query", type: "string", required: true, description: "What to recall" }],
};
...
function buildDraft(t: Tool): any {
  ...
  return {
    ...
    fields: t.kind === "builtin" ? (BUILTIN_ARG_FIELDS[c.builtin] || []) : (req.fields || c.variables || []).map((f: any) => ({ ...f })),
